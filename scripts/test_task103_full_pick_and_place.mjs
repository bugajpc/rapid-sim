...
console.log("=== TEST PEŁNEGO CYKLU PRZENOSZENIA 6 KLOCKÓW W ZADANIU ELM.08-103 ===");

const task103 = tasks.find(t => t.id === "task-elm08-103");
if (!task103) {
  console.error("Nie znaleziono zadania task-elm08-103!");
  process.exit(1);
}

console.log(`Liczba zdefiniowanych klocków w zadaniu: ${task103.blocks.length}`);
task103.blocks.forEach((b, i) => {
  console.log(`  Klocek ${i + 1} (${b.id}): pozycja = [${b.position.join(", ")}]`);
});

if (task103.blocks.length !== 6) {
  console.error("❌ BŁĄD: Zadanie 103 powinno mieć dokładnie 6 klocków!");
  process.exit(1);
}
...
const solCode = codeBlocks[7];

console.log("\nKompilacja programu wzorcowego ELM.08-103...");
...
// Symulacja z pełną fizyką chwytaka i bloków
let blocks = task103.blocks.map(b => ({ ...b, position: [...b.position] }));
let heldBlockId = null;
const variables = { ...(compiled.initialVariables || {}) };
const inputs = { S1: true };
const outputs = { H1: false, H2: false, doGripper: false };
let tcp = [...defaultTcp];
...
for (let pc = 0; pc < compiled.commands.length && steps < maxSteps; pc++) {
  ...
  if (cmd.type === "output") {
    ...
    if (cmd.signal === "doGripper") {
      if (cmd.value) {
        // Gripper CLOSE: find closest block within 70mm
        let closest = null;
        ...
        if (closest && minDist <= 70) {
          ...
          console.log(`  ➔ CHWYTAK ZACIŚNIĘTY: Pobrany klocek [${closest.id}] w punkcie [${tcp.map(Math.round).join(", ")}] (odległość: ${minDist.toFixed(1)} mm)`);
        } else {
          console.error(`  ❌ BŁĄD: Chwytak zamknięty na pusto! Brak klocka w odległości <= 70mm od TCP [${tcp.join(", ")}]. Najbliższy: dist = ${minDist.toFixed(1)} mm`);
          process.exit(1);
        }
      } else if (heldBlockId) {
        // Gripper OPEN: drop block
        const b = blocks.find(x => x.id === heldBlockId);
        if (b) {
          b.position = [...tcp];
          placeEvents.push({ id: b.id, atTcp: [...tcp] });
          console.log(`  ➔ CHWYTAK OTWARTY: Odłożony klocek [${b.id}] w punkcie [${tcp.map(Math.round).join(", ")}]`);
        }
        heldBlockId = null;
      }
    }
...
console.log(`Pobranych klocków: ${pickEvents.length} / 6`);
console.log(`Odłożonych klocków: ${placeEvents.length} / 6`);

if (pickEvents.length === 6 && placeEvents.length === 6) {
  console.log("\n🎉 TEST ZALICZONY W 100%! Wszystkie 6 klocków zostało poprawnie pobranych z unikalnych pozycji i odłożonych!");
} else {
  console.error("\n❌ TEST NIE ZALICZONY!");
  process.exit(1);
}
